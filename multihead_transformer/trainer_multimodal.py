...
from dataloader import Flickr8KDataset, QuickRecipeDataset
...
def generate_evaluation_predictions(subset, encoder, decoder, config, device):
    """Evaluates (BLEU score) caption generation model on a given subset.
    ...
    for x_img, y_caption in subset.inference_batch(batch_size):
        ...
        for predicted_words, summary_words in zip(predictions, y_caption):
            summary_words = summary_words[0]
...
def train(config, writer, device):
   ...
   # Define dataloader hyper-parameters
    ...
    # Create dataloaders
    train_set = QuickRecipeDataset(config, config["split_save"]["train"], training=True, multimodal=True)
    valid_set = QuickRecipeDataset(config, config["split_save"]["validation"], training=False, multimodal=True)
    ...
    for epoch in range(train_config["num_of_epochs"]):
        ...
        for x_img, x_words, y, tgt_padding_mask in train_loader:
            ...
            # Extract image features
            with torch.no_grad():
                # img_features = encoder(x_img) # B, C, H, W = (B, 2048, 8, 8)
                # img_features = img_features.view(img_features.size(0), img_features.size(1), -1)
                # img_features = img_features.permute(0, 2, 1) # (B, 64, 2048)
                # img_features = img_features.detach()
                img_features = x_img # (B, 1, C)
        ...
        if (epoch + 1) % train_config["eval_period"] == 0:
            ...
            save_checkpoint(decoder, optimizer, start_time, epoch, str(MODEL_SAVE_DIR))
