...
class QuickRecipeDataset(Dataset):
    """"Represents dataloader for the Quick Recipe dataset.
    ...
    def __init__(self, config, path, training=True, multimodal=False):
        """Initializes the module.
        ...
        self._df = pd.read_pickle(dataset_path)
    ...
    def _load_and_prepare_image(self, image_name, include_text_features=False):
        """Performs image preprocessing.
        ...
        row = self._df.iloc[image_name]
        resnet_features = torch.FloatTensor(np.asarray(row["img_feat"], dtype=np.float32)) # 512
        if include_text_features:
            text_features = torch.FloatTensor(np.asarray(row["text_feat(all-MiniLM-L6-v2)"], dtype=np.float32)) # 384
            tensor = torch.cat([resnet_features, text_features]) # 896
        else:
            tensor = resnet_features

        tensor = tensor.unsqueeze(dim=0) # (1, dims)
    ...
    def inference_batch(self, batch_size):
        ...
        for idx in range(num_batches):
            ...
            for image_name, captions in caption_samples:
                batch_captions.append(captions)
                batch_imgs.append(self._load_and_prepare_image(image_name, include_text_features=self._multimodal))
    ...
    def __getitem__(self, index):
        ...
        # Load and preprocess image
        image_tensor = self._load_and_prepare_image(image_id, include_text_features=self._multimodal)
